...
class Basic extends Component {
	...
	addCoinList(){
		...
		temp.push({'contractAddress' : '' ,  'symbol' : '' , 'name' : '' , 'nameKR' : ''});
  ...
  render() {
		...
		for( var item of Object.keys(this.state.rewordCoin)){		
			...
			for(var coinItem of this.state.coinList){			//코인 리스트
				rewordCoinObject[item].push({ value: coinItem.symbol, label: coinItem.name+'('+coinItem.nameKR+')' , class: 'coinList' , name : item });
				if(coinItem.symbol == this.state.rewordCoin[item]){
			...
			coinList.push(
					<Row>
						<FormInputs
							...
							arrayNum = {num}
							filedName = 'coinList'
							ncols={["col-md-4" ,  "col-md-2" , "col-md-2" , "col-md-2" ]}
							proprieties={[
								{
